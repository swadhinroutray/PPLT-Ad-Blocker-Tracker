...
output_folder = "../data/data_Ublock_Edge/";
...
class CreateHar(object):
    ...
    def __start_driver(self):
        ...
        options.add_extension('../plugins/Ublock1.45.2_8.crx')
        options.add_argument('--ignore-ssl-errors=yes')
        options.add_argument('--ignore-certificate-errors')
        # profile = webdriver.Chrome()
        # options.binary_location= './Microsoft Edge'
        options.add_argument("--proxy-server={0}".format(self.proxy.proxy))
        self.driver = webdriver.Edge('./msedgedriver',options=options)
